...
vector<AusVoteSystem::RoundResult> AusVoteSystem::ComputeResult() const {
	...
	while(true){
		...
		for(int i = 0; i < voters.size(); i++){
			if(voters[i].index >= candidates.size()) 
				continue;
		...
		for(int i = 0; i < roundresult_.size(); i++){
			...
			if(minimum > roundresult_[i].votes)
				minimum = roundresult_[i].votes;
		}

		for(int i = 0; i < roundresult_.size(); i++){
			if(roundresult_[i].votes == minimum){
				int temp = i;
				for(int j = 0; j < deleted.size(); j++){
					if(deleted[j] <= temp)
						temp++;
		...
		for(int i = 0; i < voters.size(); i++){
			while(true){
				bool ok = false;
				for(int j = 0; j < loser.size(); j++){
					if (voters[i].vote[voters[i].index] - 1 == loser[j])
						ok = true;
				}
				if(ok == true)
					voters[i].index++;
		...
		if(roundresult[roundresult.size() - 1].size() < 1){
			break;
		}
		if(roundresult[roundresult.size() - 1].size() == 1){
